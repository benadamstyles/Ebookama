...
const log = console.log,
  ...
  srcFilePath = nodeArgs.length
    ? nodeArgs[0]
    : glob.sync('*.epub').find(name => name.substr(0, 4) !== 'old-'),
...
\t\t<reference href="Text/${data.start_reading_file}#${data.start_reading_id ||
...
fs.createReadStream(srcFilePath).pipe(unzip.Parse()).on('entry', entry => {
...
process.on('exit', () => {
  try {
    ...
    log(child.execSync(`./kindlegen ${newFileName}`).toString())
